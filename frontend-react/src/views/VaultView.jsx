...
import { Lock, Key, Folder, AlertTriangle, ShieldCheck, Filter, RefreshCw, Eye } from 'lucide-react';
...
const VaultView = () => {
  ...
  const handleRotate = (id) => {
    setContainers((prev) =>
      prev.map((c) =>
        c.id === id
          ? { ...c, tag: 'PROD_ENV', isDue: false, statusText: 'ROTATED: JUST NOW', buttonText: 'VIEW' }
          : c
      )
    );
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, backgroundColor: '#000000' }}>
      {/* Top Card: AUTHENTICATION STATUS (Matching Vault Image) */}
      ...
      {/* Main Grid: Encrypted Containers (Left) vs Controls & Access Log (Right) */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '16px' }}>
        {/* Left Column: ENCRYPTED CONTAINERS Grid */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {/* 2x2 Grid of Vault Containers */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {containers.map((item) => (
              <div
                key={item.id}
                style={{
                  background: item.isDue ? 'rgba(255, 68, 34, 0.04)' : '#111215',
                  border: item.isDue ? '2px solid #ff4422' : '1px solid #22242a',
                  borderRadius: '2px',
                  padding: '16px',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span
                    ...
                    style={{
                      ...
                      color: item.isDue ? '#ff4422' : '#888888',
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#555866', marginTop: '6px', letterSpacing: '2px' }}>
                    {item.value}
                  </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #1a1c22', paddingTop: '10px' }}>
                  <span style={{ fontSize: '0.65rem', color: item.isDue ? '#ff4422' : 'var(--text-muted)', fontWeight: item.isDue ? 700 : 400 }}>
                    {item.statusText}
                  </span>

                  <button
                    onClick={() => item.isDue && handleRotate(item.id)}
                    style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div className="tactical-panel" style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <button className="btn-tactical btn-override" style={{ width: '100%', justifyContent: 'center', marginTop: '6px' }}>
                APPLY POLICIES
              </button>
            </div>
          </div>
